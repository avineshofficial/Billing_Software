...
const InvoiceTicket = forwardRef(({ 
  ...
  return (
    <div ref={ref} style={{ 
      ...
      width: '300px', // Increased width from 210px to 300px
      ...
      <style>{`
        @media print {
          @page { margin: 0; size: 80mm auto; } /* Changed page size to 80mm */
          body { margin: 0; padding: 0; -webkit-print-color-adjust: exact; }
        }
      `}</style>

      {/* --- STORE HEADER (TAMIL) --- */}
      <h2 style={{ textAlign: 'center', margin: '10px 0 2px 0', fontSize: '22px', fontWeight: '900' }}>நெருங்கிய கூட்டாளி</h2>
      <p style={{ textAlign: 'center', margin: '0', fontSize: '16px', fontWeight: '900' }}>நிறுவனம்</p>
      ...
      <table style={{ width: '100%', fontSize: '11px', borderCollapse: 'collapse', fontWeight: '900', tableLayout: 'fixed' }}>
        ...
        <tbody>
          {cart.map((item, index) => {
            ...
            return (
              <tr key={index}>
                <td style={{ padding: '6px 0', textTransform: 'uppercase', fontSize: '10px', wordBreak: 'break-all', verticalAlign: 'top' }}>
                    {item.name}
                </td>
      ...
      <div style={{ fontSize: '13px', fontWeight: '900' }}>
        ...
        <div style={{ borderTop: '3px solid #000', marginTop: '8px', paddingTop: '5px', display: 'flex', justifyContent: 'space-between', fontSize: '18px' }}>
